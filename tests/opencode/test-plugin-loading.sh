#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the super plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/super.js"
...
skill_count=$(find "$SUPER_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SUPER_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-super skill exists (critical for bootstrap)
echo "Test 3: Checking using-super skill (required for bootstrap)..."
if [ -f "$SUPER_SKILLS_DIR/using-super/SKILL.md" ]; then
    echo "  [PASS] using-super skill exists"
else
    echo "  [FAIL] using-super skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SUPER_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/super/' "$SUPER_PLUGIN_FILE"; then
