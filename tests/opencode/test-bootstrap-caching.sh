...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPER_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SUPER_SKILLS_DIR/using-super/SKILL.md" "$TEST_HOME/using-super.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPER_PLUGIN_FILE" missing
}
