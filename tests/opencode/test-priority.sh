...
# 1. Create in super location (lowest priority)
mkdir -p "$SUPER_SKILLS_DIR/priority-test"
cat > "$SUPER_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Super version of priority test skill
---
# Priority Test Skill (Super Version)

This is the SUPER version of the priority test skill.

PRIORITY_MARKER_SUPER_VERSION
EOF
...
if [ -f "$SUPER_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Super version exists"
else
    echo "  [FAIL] Super version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled super priority
echo ""
echo "Test 2: Documenting personal vs super priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled super version instead of personal version"

# Test 3: Document project vs bundled super priority
echo ""
echo "Test 3: Documenting project vs personal/super priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled super version instead of project version"

# Test 4: Test a non-colliding bundled super skill is still available
echo ""
echo "Test 4: Testing non-colliding super skill remains available..."

mkdir -p "$SUPER_SKILLS_DIR/super-only-test"
cat > "$SUPER_SKILLS_DIR/super-only-test/SKILL.md" <<'EOF'
---
name: super-only-test
description: Super-only priority test skill
---
# Super Only Test Skill

PRIORITY_MARKER_SUPER_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"super-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPER_ONLY_VERSION" "Non-colliding super skill is still registered"
