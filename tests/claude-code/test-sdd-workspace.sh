...
main() {
    ...
    if [[ "$dir" == "$repo/.super/sdd" ]]; then
        pass "prints <repo-root>/.super/sdd"
    else
        fail "prints <repo-root>/.super/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.super/sdd/.gitignore" && "$(cat "$repo/.super/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.super/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.super/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.super/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.super/sdd" && "$wt_dir" != "$dir" ]]; then
