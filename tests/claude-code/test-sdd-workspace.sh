...
main() {
    ...
    if [[ "$dir" == "$repo/.super-workspace/sdd" ]]; then
        pass "prints <repo-root>/.super-workspace/sdd"
    else
        fail "prints <repo-root>/.super-workspace/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.super-workspace/sdd/.gitignore" && "$(cat "$repo/.super-workspace/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.super-workspace/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.super-workspace/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.super-workspace/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.super-workspace/sdd" && "$wt_dir" != "$dir" ]]; then
    ...
    printf 'y\n' > "$wt/.super-workspace/sdd/artifact.md"
    local wt_status
