...
EOF
    ...
    cat > "$repo/assets/super-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/super/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/super/.fixture-keep"
    cat > "$repo/plugins/super/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/super/.fixture-keep
    git -C "$repo" add plugins/super/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/super/skills/example/agents"

    cat > "$repo/plugins/super/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/super/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/super/.codex-plugin" \
        "$repo/plugins/super/.private-journal" \
        "$repo/plugins/super/assets" \
        "$repo/plugins/super/hooks" \
        "$repo/plugins/super/skills/example/agents" \
        "$repo/plugins/super/skills/example"

    cat > "$repo/plugins/super/.codex-plugin/plugin.json" <<EOF
{
  "name": "super",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/super/assets/super-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/super/assets/app-icon.png"

    cat > "$repo/plugins/super/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/super/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/super/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/super/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/super/hooks/session-start" "$repo/plugins/super/hooks/session-start-codex" "$repo/plugins/super/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/super/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/super/.codex-plugin/plugin.json \
        plugins/super/assets/app-icon.png \
        plugins/super/assets/super-small.svg \
        plugins/super/hooks/hooks-codex.json \
        plugins/super/hooks/run-hook.cmd \
        plugins/super/hooks/session-start \
        plugins/super/hooks/session-start-codex \
        plugins/super/skills/example/agents/openai.yaml \
        plugins/super/skills/example/SKILL.md \
        plugins/super/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/super/.kimi-plugin" \
        "$repo/plugins/super/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/super/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/super/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/super/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/super/.fixture-keep \
        plugins/super/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/super/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/super/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/super-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (super-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/super-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/super/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/super" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/super-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/super'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/super-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/super-*" "Clean no-op local apply does not create sync branch in destination checkout"
