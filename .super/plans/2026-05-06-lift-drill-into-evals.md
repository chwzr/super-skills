# Lift drill into super as `evals/` — implementation plan
...
**Goal:** Move the standalone `obra/drill` skill-compliance benchmark into super as a top-level `evals/` directory, delete redundant bash tests under `super/tests/` after per-file subagent verification of drill scenario coverage, and update top-level docs so contributors land on the new structure.

**Architecture:** Single PR against `dev` on a new branch `f/evals-lift`. Drill source is copied verbatim with explicit rsync excludes to keep `.git/`, `.venv/`, etc. out of the new dir. A small helper in `drill/cli.py` defaults `SUPER_ROOT` to the parent of the `evals/` directory, so contributors don't have to set the env var. Each bash-test deletion is gated by a subagent that compares the bash test's assertions to its claimed drill scenario's verify block. Historical references in plan docs and release notes are annotated, not rewritten.

**Tech Stack:** Python 3.11 + uv (drill's existing toolchain, unchanged); rsync; bash; git.

**Spec:** `.super/specs/2026-05-06-lift-drill-into-evals-design.md` — read this first.

**Drill source location:** `/Users/jesse/Documents/GitHub/super/drill/` (sibling to `super/`).
...
```bash
cd /Users/jesse/Documents/GitHub/super/super
git status --short
```
...
```bash
cd /Users/jesse/Documents/GitHub/super/drill
...
```bash
cd /Users/jesse/Documents/GitHub/super/super
test -d /Users/jesse/Documents/GitHub/super/drill && echo "drill source: OK"
...
rsync -a \
  ...
  --exclude='*.pyc' \
  /Users/jesse/Documents/GitHub/super/drill/ \
  evals/
```
...
rsync of obra/drill@$DRILL_SHA into super/evals/, excluding
...
while read -r f; do
  ...
  sha2=$(shasum -a 256 "/Users/jesse/Documents/GitHub/super/super/evals/${f#./}" | cut -d' ' -f1)
  if [ "$sha1" != "$sha2" ]; then
...
```bash
cd /Users/jesse/Documents/GitHub/super/super/evals
uv sync
```
...
Expected: starts with scenario names. (Will likely error or warn about missing SUPER_ROOT — that's fine, fixed in next task.)
...
/Users/jesse/Documents/GitHub/super/drill into
/Users/jesse/Documents/GitHub/super/super/evals.

Verify:

1. The lift commit message records the SHA reported by:
  cd /Users/jesse/Documents/GitHub/super/drill && git rev-parse HEAD
...
## Task 5: Add `SUPER_ROOT` default helper
...
```bash
sed -n '1,20p' /Users/jesse/Documents/GitHub/super/super/evals/drill/cli.py
```
...
```python
def test_set_super_root_default_when_unset(monkeypatch, tmp_path):
    """When SUPER_ROOT is unset, helper sets it to PROJECT_ROOT.parent."""
    monkeypatch.delenv("SUPER_ROOT", raising=False)
    from drill.cli import _set_super_root_default, PROJECT_ROOT

    _set_super_root_default()

    import os
    assert os.environ["SUPER_ROOT"] == str(PROJECT_ROOT.parent)


def test_set_super_root_default_respects_existing(monkeypatch):
    """When SUPER_ROOT is already set, helper does not override."""
    monkeypatch.setenv("SUPER_ROOT", "/custom/path")
    from drill.cli import _set_super_root_default

    _set_super_root_default()

    import os
    assert os.environ["SUPER_ROOT"] == "/custom/path"
```
...
```bash
cd /Users/jesse/Documents/GitHub/super/super/evals
uv run pytest tests/test_cli.py -k set_super_root_default -v
```

Expected: 2 tests fail with `AttributeError: module 'drill.cli' has no attribute '_set_super_root_default'`.

- [ ] **Step 4: Add the helper to cli.py**

Edit `/Users/jesse/Documents/GitHub/super/super/evals/drill/cli.py`. Replace lines 1–14 with:
...
def _set_super_root_default() -> None:
    """Default SUPER_ROOT to the parent of evals/ if not already set.

    Drill historically required contributors to export SUPER_ROOT
    pointing at the super checkout. After lifting drill into
    super/evals/, the parent of PROJECT_ROOT is always the
    super root, so we can supply this default automatically.

    Existing SUPER_ROOT environment values are respected as overrides.
    """
    os.environ.setdefault("SUPER_ROOT", str(PROJECT_ROOT.parent))


_set_super_root_default()
```

The bottom-of-module call to `_set_super_root_default()` runs at import time, immediately after `load_dotenv()`. This ensures both `engine.py` and `setup.py` (which read `os.environ["SUPER_ROOT"]` directly) and the YAML interpolation (which reads `os.environ` when the backend YAML is loaded) all see the value.
...
git commit -m "evals: default SUPER_ROOT to parent of evals/ if unset

Adds _set_super_root_default() to drill/cli.py, called at
module import after load_dotenv(). PROJECT_ROOT resolves to evals/
post-lift; its parent is the super repo root, which is the
correct value for SUPER_ROOT.
...
**Files:**
- Modify: `evals/backends/codex.yaml` (drop `SUPER_ROOT` from `required_env`)
- Modify: `evals/backends/gemini.yaml` (drop `SUPER_ROOT` from `required_env`)

The five `claude*.yaml` backend configs interpolate `${SUPER_ROOT}` into `args` for the `--plugin-dir` flag — they keep `SUPER_ROOT` in `required_env` because the interpolation needs it. The codex/gemini configs only listed it for engine.py/setup.py's `os.environ` reads, which the helper now satisfies.

- [ ] **Step 1: Confirm current state**

```bash
grep -A3 'required_env:' /Users/jesse/Documents/GitHub/super/super/evals/backends/codex.yaml
grep -A2 'required_env:' /Users/jesse/Documents/GitHub/super/super/evals/backends/gemini.yaml
```

Expected outputs include `- SUPER_ROOT` lines.

- [ ] **Step 2: Read codex.yaml fully**

```bash
cat /Users/jesse/Documents/GitHub/super/super/evals/backends/codex.yaml
```

- [ ] **Step 3: Edit codex.yaml — drop the `- SUPER_ROOT` line under `required_env`**
...
- [ ] **Step 4: Edit gemini.yaml — drop the `- SUPER_ROOT` line under `required_env`**
...
git commit -m "evals: drop SUPER_ROOT from codex/gemini required_env

These backends only read SUPER_ROOT via engine.py/setup.py's
os.environ access, which the new cli.py default helper supplies
automatically. claude*.yaml keep SUPER_ROOT in required_env
because they interpolate \${SUPER_ROOT} into --plugin-dir args."
```
...
For tests that assert `SUPER_ROOT` membership in `codex.yaml`'s or `gemini.yaml`'s `required_env`: invert the assertion to confirm absence. Example:

```python
# Before:
def test_codex_requires_super_root():
    backend = load_backend("codex")
    assert "SUPER_ROOT" in backend.required_env

# After:
def test_codex_does_not_require_super_root():
    """codex.yaml dropped SUPER_ROOT from required_env;
    the cli.py helper supplies the default."""
    backend = load_backend("codex")
    assert "SUPER_ROOT" not in backend.required_env
```
...
**Files:**
- Modify: `evals/README.md` (drop SUPER_ROOT setup step)
- Modify: `evals/CLAUDE.md` (drop SUPER_ROOT setup step)
...
```bash
export SUPER_ROOT=/path/to/super
...
`SUPER_ROOT` defaults to the parent of `evals/` (the super repo root) and only needs to be set if you're running drill against a different super checkout.
```
...
`SUPER_ROOT` defaults to the parent of `evals/` (the super repo root). Override only if running drill against a different super checkout.
```
...
git commit -m "evals: drop SUPER_ROOT setup step from README/CLAUDE
...
Expected: scenario list, no error about missing SUPER_ROOT.
...
If FAIL, debug before continuing. The path-defaults change is the most likely culprit; check that the helper actually fired by adding a `print(os.environ["SUPER_ROOT"])` after the helper call temporarily.
...
```bash
ls /Users/jesse/Documents/GitHub/super/super/tests/skill-triggering/prompts/
```
...
```bash
for f in /Users/jesse/Documents/GitHub/super/super/tests/explicit-skill-requests/*.sh /Users/jesse/Documents/GitHub/super/super/tests/explicit-skill-requests/prompts/*.txt; do
  echo "=== $f ==="
  cat "$f" | head -30
done
```
...
```bash
ls /Users/jesse/Documents/GitHub/super/super/evals/fixtures/sdd-go-fractals/
ls /Users/jesse/Documents/GitHub/super/super/evals/fixtures/sdd-svelte-todo/
```
...
```bash
cat /Users/jesse/Documents/GitHub/super/super/tests/claude-code/test-subagent-driven-development.sh
```
...
- Annotate (do not rewrite): `RELEASE-NOTES.md`, `.super/plans/*.md`
...
| `.super/plans/*.md` | Annotate, don't rewrite (dated artifact) |
...
For each `RELEASE-NOTES.md` or `.super/plans/*.md` hit, add an inline annotation at the *first* hit per file:
...
```
Working directory: /Users/jesse/Documents/GitHub/super/super
...
Search the entire super tree (excluding evals/, node_modules/,
...
and .super/plans/*.md are annotated as dated artifacts,
not rewritten."
```
...
Open `/Users/jesse/Documents/GitHub/super/super/docs/testing.md` and replace the file content with this structure (preserve the existing Plugin-test details where applicable):

```markdown
# Testing Super

Super has two distinct kinds of tests, each in its own directory:
...
Open `/Users/jesse/Documents/GitHub/super/super/.gitignore` and add at the bottom:
...
```bash
cd /Users/jesse/Documents/GitHub/super/super/tests/brainstorm-server
...
**Branch:** f/evals-lift, in /Users/jesse/Documents/GitHub/super/super
**Base:** dev (currently b4363df)
**Spec:** .super/specs/2026-05-06-lift-drill-into-evals-design.md

This branch lifts the obra/drill repo into super/evals/ and
...
3. Does the SUPER_ROOT helper actually default correctly when
   the env var is unset? (cd evals && unset SUPER_ROOT && uv
...
gh pr create \
  ...
  --head f/evals-lift \
  --reviewer arittr \
  --title "Lift drill into super as evals/ harness" \
  --body "$(cat <<'EOF'
## What problem are you trying to solve?

Drill — the standalone Python skill-compliance benchmark at obra/drill — is already the de facto eval harness for super. The PRI-1397 commit series lifted ~22 bash tests into drill scenarios, and the most recent super commit (a2292c5) explicitly removed a redundant bash test with the message "replaced by drill behavioral coverage". Drill is a sibling repo today, requiring contributors to clone two checkouts and set SUPER_ROOT manually. This PR completes the migration: drill becomes super/evals/.

## What does this PR change?

- Lifts the obra/drill repo into super as `evals/`, with explicit rsync excludes (.git, .venv, results, .env, __pycache__, *.egg-info, .private-journal). The lift commit records the source SHA.
- Adds a `_set_super_root_default()` helper to drill/cli.py so SUPER_ROOT defaults to the parent of evals/ — no manual env-var setup.
- Drops SUPER_ROOT from required_env in codex.yaml/gemini.yaml (the helper supplies it). Claude*.yaml keep it because they interpolate ${SUPER_ROOT} into --plugin-dir args.
...
Yes. Cross-runtime evaluation is core to super, the migration to drill scenarios was already underway in this repo, and the eval harness needs to be discoverable in-tree to be findable.

## What alternatives did you consider?

- Vendored copy + sync script (drill repo continues independently). Rejected: divergence risk; single-source-of-truth wins.
- git subtree merge (preserves drill history in-tree). Rejected: super' git history grows by 50+ commits, the merge commit is ugly, subtrees are operationally heavy.
- Keep drill as a sibling repo and just polish docs. Rejected: doesn't solve the discoverability problem.

## Does this PR contain multiple unrelated changes?

No — every change supports "drill is now evals/ inside super". Multiple commits for atomicity (verbatim copy, env helper, YAML updates, docs) but one direction.

## Existing PRs

- [x] I have reviewed all open AND closed PRs for duplicates or prior art
- Related PRs: #1486 (chwzr/super-skills cross-platform PR — independent; no shared file changes besides README, which has no overlap)
...
- Initial prompt: see linked spec (`.super/specs/2026-05-06-lift-drill-into-evals-design.md`).
- Drill's own pytest suite passes.
- One drill scenario re-run from the new location end-to-end (proves the SUPER_ROOT default works).
...
1. Archive obra/drill on GitHub (mark read-only, add README pointer to chwzr/super-skills/evals/).
...
- [ ] `cd evals && unset SUPER_ROOT && uv run pytest` passes
- [ ] `cd evals && unset SUPER_ROOT && uv run drill list` returns scenarios
- [ ] `cd evals && unset SUPER_ROOT && uv run drill run triggering-test-driven-development -b claude` passes
- [ ] `tests/brainstorm-server/server.test.js` still passes (regression gate for non-LLM tests)
- [ ] `git diff dev..HEAD .super/plans/2026-04-06-worktree-rototill.md .super/plans/2026-03-23-codex-app-compatibility.md RELEASE-NOTES.md` shows annotations only, no path rewrites
