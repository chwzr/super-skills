...
**Spec:** `.super/specs/2026-06-09-sdd-task-scoped-review-dispatch-design.md` — read it before starting. Decisions already settled there: full re-reviews stay; the two review stages stay separate; coordinator keeps model judgment; `requesting-code-review/` stays broad.
...
```
- [code-quality-reviewer-prompt.md](code-quality-reviewer-prompt.md) - Dispatch code quality reviewer subagent
- Final whole-branch review: use super:requesting-code-review's [code-reviewer.md](../requesting-code-review/code-reviewer.md)
```
...
```
- **super:requesting-code-review** - Code review template for the final whole-branch review
```
...
Lives in the `evals/` **submodule** (separate repo, `super-evals`). Work on a branch there; the parent submodule-pointer bump happens at finishing time per `evals/CLAUDE.md`.
...
def scaffold_sdd_quality_defect_plan(workdir: Path) -> None:
    ...
    plans_dir = workdir / "docs" / "super" / "plans"
...
You have a small plan at .super/plans/report-plan.md — two report
...
"I have a small plan at .super/plans/report-plan.md — two report
formatting functions. Use the super:subagent-driven-development skill
...
- A `Skill` invocation naming `super:subagent-driven-development`
...
pre() {
    ...
    git-branch main
    requires-tool npm
    file-exists '.super/plans/report-plan.md'
    file-contains '.super/plans/report-plan.md' 'formatAdminReport'
    file-contains '.super/plans/report-plan.md' 'repeat\(40\)'
}

post() {
    skill-called super:subagent-driven-development
    tool-called Agent
...
Run: `grep -n "code-quality-reviewer" skills/using-super/references/antigravity-tools.md skills/using-super/references/gemini-tools.md`
...
- [ ] **Step 1: Baseline (skills as released on dev)** — from the main checkout (`/Users/jesse/git/super/super`, on dev), or any checkout without this branch's changes:

```bash
cd evals
export SUPER_ROOT=/Users/jesse/git/super/super
...
- [ ] **Step 2: After (this branch's skills)** — point `SUPER_ROOT` at this worktree:

```bash
cd evals
export SUPER_ROOT=/Users/jesse/git/super/super/.claude/worktrees/sdd-review-dispatch
uv run quorum run scenarios/sdd-rejects-extra-features --coding-agent claude
uv run quorum run scenarios/sdd-go-fractals --coding-agent claude
uv run quorum run scenarios/sdd-svelte-todo --coding-agent claude
uv run quorum run scenarios/spec-reviewer-catches-planted-flaws --coding-agent claude
...
After all tasks pass: the evals submodule commit needs to land in `super-evals` (PR to its `main`), then this branch bumps the `evals` submodule pointer — per `evals/CLAUDE.md`, the parent bump is part of propagation, not optional. Then use super:finishing-a-development-branch. PRs against super target `dev`.
