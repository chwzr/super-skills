...
> **For agentic workers:** REQUIRED SUB-SKILL: Use super:executing-plans to implement this plan task-by-task.

**Goal:** Add full super support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/super-codex` (lines 40-74)
...
- Reference: `.codex/super-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'super' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/super-codex` (lines 212-280)
 ...
 * (personal skills override super skills).
 *
 * @param {string} skillName - Name like "super:brainstorming" or "my-skill"
 * @param {string} superDir - Path to super skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superDir, personalDir) {
    // Strip super: prefix if present
    const forceSuper = skillName.startsWith('super:');
    const actualSkillName = forceSuper ? skillName.replace(/^super:/, '') : skillName;

    // Try personal skills first (unless explicitly super:)
    if (!forceSuper && personalDir) {
    ...
    // Try super skills
    if (superDir) {
        const superPath = path.join(superDir, actualSkillName);
        const superSkillFile = path.join(superPath, 'SKILL.md');
        if (fs.existsSync(superSkillFile)) {
            return {
                skillFile: superSkillFile,
                sourceType: 'super',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/super-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/super-codex` (add import at top)
...
Run: `node -c .codex/super-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/super-codex
...
**Files:**
- Modify: `.codex/super-codex` (lines 40-74)
...
Run: `.codex/super-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/super-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/super-codex` (lines 16-38, approximately)
...
Run: `.codex/super-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/super.js`
...
const superSkillsDir = path.join(homeDir, '.config/opencode/super/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SuperPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/super.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/super.js
...
**Files:**
- Modify: `.opencode/plugin/super.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "super:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > super)
...
Run: `node -c .opencode/plugin/super.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the super and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const superSkills = skillsCore.findSkillsInDir(
            superSkillsDir,
            'super',
            3
          );
          ...
          const allSkills = [...personalSkills, ...superSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install super skills to ~/.config/opencode/super/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'super:';
...
```javascript
    'session.started': async () => {
      // Read using-super skill content
      const usingSuperPath = skillsCore.resolveSkillPath(
        'using-super',
        superSkillsDir,
        personalSkillsDir
      );

      let usingSuperContent = '';
      if (usingSuperPath) {
        const fullContent = fs.readFileSync(usingSuperPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSuperContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Super skills: \`super:skill-name\` (from ~/.config/opencode/super/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/super')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/super && git pull` to update super.' :
        '';
...
**Below is the full content of your 'super:using-super' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
### 1. Install Super Skills

```bash
# Clone super skills to OpenCode config directory
mkdir -p ~/.config/opencode/super
git clone https://github.com/chwzr/super-skills.git ~/.config/opencode/super
```

### 2. Install the Plugin

The plugin is included in the super repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/super/.opencode/plugin/super.js`
...
ln -s ~/.config/opencode/super/.opencode/plugin/super.js .opencode/plugin/super.js
```
...
```
use use_skill tool with skill_name: "super:brainstorming"
```
...
Personal skills override super skills with the same name.

## Updating

```bash
cd ~/.config/opencode/super
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/super/.opencode/plugin/super.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/super/skills`
...
- Report issues: https://github.com/chwzr/super-skills/issues
- Documentation: https://github.com/chwzr/super-skills
```
...
Super works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/super-codex use-skill super:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/super-codex bootstrap | head -30`
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/super.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/super-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/super-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/super.js` created with tools and hooks
