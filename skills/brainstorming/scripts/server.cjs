...
const SUPER_VERSION = readSuperVersion();
const SUPER_BRAND_IMAGE_URL = 'https://telemetry.flxkpe.io/super-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'SUPER_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const SUPER_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(SUPER_VERSION);
  const text = 'Super v' + version;
  const logo = SUPER_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + SUPER_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(SUPER_VERSION) + '" alt="Super" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/chwzr/super-skills">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
